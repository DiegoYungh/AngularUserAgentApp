...
class WsseListener implements ListenerInterface
{
    ...
    public function handle(GetResponseEvent $event)
    {
        ...
        #$response = new Response(json_encode($request->headers->has('x-wsse')));
        #$response = new Response(json_encode($request->headers->get('x-wsse')));
        #$response = new Response(json_encode($request->headers->all()));
        $response->setStatusCode(403);
        $event->setResponse($response);
